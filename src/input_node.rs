...
impl InputNode {
    ...
    async fn forward_to_backend(
        ...
        if let Some(api_key) = backend.api_key.as_deref() {
            ...
            if payload.parts.headers.contains_key("x-api-key") {
                req_builder = req_builder.header("x-api-key", api_key);
                skip_header.insert(HeaderName::from_static("x-api-key"));
                api_key_added = true
            }
        ...
        // 发送请求到后端
        return match client.request(forward_req).await {
            Ok(response) => {
